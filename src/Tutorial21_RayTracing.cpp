...
void Tutorial21_RayTracing::CreateRayTracingPSO()
{
    ...
    RefCntAutoPtr<IShader> pCubePrimaryHit, pGroundHit, pGlassPrimaryHit, pSpherePrimaryHit, pSphereSolidHit, pSphereVidrioHit, pCubeSolidHit, pCubeMetalHit;
    {
        ...
        ShaderCI.Desc.Name  = "Cube primary ray closest hit shader";
        ShaderCI.FilePath   = "CubeSolidHit.rchit";
        ShaderCI.EntryPoint = "main";
        m_pDevice->CreateShader(ShaderCI, &pCubeSolidHit);
        VERIFY_EXPR(pCubeSolidHit != nullptr);

        ShaderCI.Desc.Name  = "Cube metal ray closest hit shader";
        ShaderCI.FilePath   = "CubeMetalHit.rchit";
        ShaderCI.EntryPoint = "main";
        m_pDevice->CreateShader(ShaderCI, &pCubeMetalHit);
        VERIFY_EXPR(pCubeMetalHit != nullptr);
    ...
    PSOCreateInfo.AddTriangleHitShader("CubeSolidHit", pCubeSolidHit);

    PSOCreateInfo.AddTriangleHitShader("CubeMetalHit", pCubeMetalHit);
...
std::vector<float3> GenerateGridPositions(int totalObjects, float spacing, float yPos)
{
    std::vector<float3> positions;
    positions.reserve(totalObjects);

    // Calcular dimensiones de la grilla
    int gridSize = static_cast<int>(std::ceil(std::sqrt(totalObjects)));
    
    // Generar posiciones en grilla 2D
    for (int x = 0; x < gridSize; ++x)
    {
        for (int z = 0; z < gridSize; ++z)
        {
            if (positions.size() >= totalObjects)
                break;

            float posX = (x - gridSize/2.0f) * spacing;
            float posZ = (z - gridSize/2.0f) * spacing;
            positions.emplace_back(posX, yPos, posZ);
        }
    }
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    if (smallSpherePositions.empty())
    {
        ...
        const float                           rMin   = 5.0f; // ninguna esfera dentro de radio 2

        for (int i = 0; i < MaxSmallSpheres; ++i)
        {
            float x, z;
            // volvemos a muestrear si está muy cerca del centro
            do {
                x = distX(rng);
                z = distZ(rng);
            } while (x * x + z * z < rMin * rMin);
    ...
    if (smallCubePositions.empty())
    {
        ...
        for (int i = 0; i < MaxSmallSpheres; ++i)
        {
            ...
            smallCubePositions.emplace_back(x, yFloor, z);
        }
    }
    ...
    for (int k = 0; k < 3; ++k)
    {
        ...
        inst.Transform.SetTranslation(
            ...
            -4.0f,
            positions[k].z);
        inst.Transform.SetRotation(float3x3::Scale(2.5f, 2.5f, 2.5f).Data());
    }
    ...
    constexpr int CubeStartIndex = BaseCount + MaxSmallSpheres;
    for (int i = 0; i < MaxSmallCubes; ++i)
    {
        int   idx  = CubeStartIndex + i;
        auto& inst = Instances[idx];
        ...
        inst.Mask = (i < m_NumScatterCubes ? OPAQUE_GEOM_MASK : 0u);

        // Usar la posición generada
        auto pos = smallCubePositions[i];
        inst.Transform.SetTranslation(pos.x, -5.34f, pos.z);

        inst.Transform.SetRotation(float3x3::Scale(0.5f, 0.5f, 0.5f).Data());
    }
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance", PRIMARY_RAY_INDEX, "CubeMetalHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance 2", PRIMARY_RAY_INDEX, "GlassPrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance 3", PRIMARY_RAY_INDEX, "CubeSolidHit");
    // clang-format on
    ...
    for (int i = 0; i < NumSmallSpheres; ++i)
    {
        ...
        float ratio = static_cast<float>(i) / NumSmallSpheres;

        if (ratio < 0.7f)
            hitGroupName = "SphereSolidHit";
        else if (ratio < 0.85f)
            hitGroupName = "SpherePrimaryHit";
        else
            hitGroupName = "SphereVidrio";
    ...
    for (int i = 0; i < NumSmallCubes; ++i)
    {
        ...
        // Patrón cíclico cada 3 cubos
        switch (i % 3)
        {
            case 0: hitGroupName = "CubeSolidHit"; break;
            case 1: hitGroupName = "CubeMetalHit"; break;
            case 2: hitGroupName = "GlassPrimaryHit"; break;
        }
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Initialize constants.
    {
        ...
        m_Constants.GlassReflectionColorMask = {1.0f, 1.0f, 1.0f};
        m_Constants.GlassAbsorption          = 0.5f;
        m_Constants.GlassMaterialColor       = {1.0f, 1.0f, 1.0f};
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::SliderInt("Sphere Count", &m_NumScatterSpheres, 0, MaxSmallSpheres))
    {
        ...
        for (int i = 0; i < m_NumScatterSpheres; ++i)
        {
            ...
            m_ScatterSphereNames.push_back("Sphere" + std::to_string(i));
        }
    }

    ImGui::Separator();
    ImGui::Text("Cantidad cubos");
    if (ImGui::SliderInt("Cube Count", &m_NumScatterCubes, 0, MaxSmallCubes))
    {
        // reconstruyo SOLO los cubos
        ...
        for (int i = 0; i < m_NumScatterCubes; ++i)
        {
            ...
            m_ScatterCubeNames.push_back("Cube" + std::to_string(i));
        }
    }
